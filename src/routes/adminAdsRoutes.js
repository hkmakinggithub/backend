...
//   } catch (error) {
//     console.error('Get ads by location error:', error);
//     res.status(500).json([]);
//   }
// });
...
//     if (category) {
//       sql += ` AND (category_target = $2 OR category_target IS NULL)`;
//       params.push(category);
//     } else {
//       sql += ` AND category_target IS NULL`;
//     }
    
//     sql += ` ORDER BY priority ASC, RANDOM() LIMIT 1`;
...
//   } catch (error) {
//     console.error('Get home ad error:', error);
...
//   try {
//     const { title, image_url, ad_location } = req.body; 
//     const result = await query(
//       'INSERT INTO advertisements (title, image_url, ad_location) VALUES ($1, $2, $3) RETURNING *',
//       [title, image_url, ad_location]
//     );
...
//   }
// });
// // Inside your routes/adminAdsRoutes.js

// router.get('/ads/by-location', async (req, res) => {
//   try {
//     const { location } = req.query;
    
//     // THE ULTIMATE AD ENGINE QUERY
//     const result = await query(`
//       SELECT * FROM advertisements 
//       WHERE is_active = true 
      
//       -- RULE 3: WHICH DAY (Only shows if today is between start_date and end_date)
//       AND (start_date IS NULL OR start_date <= CURRENT_DATE)
//       AND (end_date IS NULL OR end_date >= CURRENT_DATE)
      
//       -- RULE 2: HOW MANY TIMES (Only shows if current impressions are less than max_impressions)
//       -- (Note: Make sure you add a 'max_impressions' column to your database!)
//       AND (max_impressions IS NULL OR impressions < max_impressions)
      
//       -- RULE 1: DIFFERENT AD EVERY TIME (Shuffles the ads directly in the database)
//       ORDER BY RANDOM() 
      
//       -- Limit to 2 ads so the mobile app doesn't crash downloading heavy data
//       LIMIT 2;
//     `);
...
router.post('/ads', upload.single('media'), async (req, res) => {
  try {
    ...
    // ✅ CRITICAL FIX: Convert empty strings to null for PostgreSQL
...
// ✅ THE ULTIMATE AD ENGINE (By Location)
router.get('/ads/by-location', async (req, res) => {
  try {
    const { location } = req.query;
    const today = new Date().toISOString().split('T')[0];
